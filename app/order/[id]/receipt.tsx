...
export default function ReceiptScreen() {
  ...
  const handleShare = async () => {
    try {
      await Share.share({
        message: `Экспедиторская расписка\nПеревозка ${id}\nДата: ${new Date().toLocaleDateString('ru-RU')}`,
      ...
      >
        <View style={[styles.documentCard, { backgroundColor: Colors.surface1 }]}>
          ...
          <View style={[styles.infoSection, { borderTopColor: Colors.border }]}>
            <Text style={[styles.infoLabel, { color: Colors.textSecondary }]}>Дата выдачи</Text>
          ...
          <View style={[styles.infoSection, { borderTopColor: Colors.border }]}>
            ...
            <Text style={[styles.infoValue, { color: Colors.text }]}>ЭР-{id}</Text>
          </View>
